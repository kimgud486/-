...
export const ToastContainer: React.FC = () => {
  ...
  return (
    <div 
      className="fixed bottom-3 right-3 sm:bottom-5 sm:right-5 z-50 flex flex-col gap-2 max-w-[320px] w-full pointer-events-none px-2"
      id="aistock-toast-notification-system"
    >
      {/* Toast Top Controller Bar */}
      <div className="pointer-events-auto flex items-center justify-between bg-slate-950/90 border border-slate-800/80 rounded-lg px-2.5 py-1 text-[10px] text-zinc-400 shadow-md backdrop-blur-md">
        <div className="flex items-center gap-1.5">
          <Bell className="h-3 w-3 text-indigo-400" />
          <span className="font-semibold text-zinc-300">실시간 알림 ({toasts.length})</span>
        </div>
      ...
      {toasts.map((toast: any) => {
        ...
        const bgClass = isSuccess 
          ? "bg-slate-950/95 border-emerald-500/50 text-white shadow-lg"
          : isError
          ? "bg-slate-950/95 border-rose-500/60 text-white shadow-lg"
          : isWarning
          ? "bg-slate-950/95 border-amber-500/50 text-white shadow-lg"
          : "bg-slate-950/95 border-indigo-500/50 text-white shadow-lg";
        ...
        return (
          <div
            key={toast.id}
            className={`pointer-events-auto border rounded-xl p-2.5 transition-all duration-200 transform translate-y-0 scale-100 flex flex-col space-y-1.5 ${bgClass} backdrop-blur-md`}
          >
              ...
              <div className="flex items-start gap-2 min-w-0">
                ...
                <div className="space-y-0.5 min-w-0">
                  <div className="flex items-center gap-1.5 flex-wrap">
                    <h4 className="text-[11px] font-bold tracking-tight truncate max-w-[180px]">{toast.title}</h4>
                    <span className="text-[9px] text-zinc-400 font-mono flex items-center gap-0.5">
                      <Clock className="h-2 w-2" />
                      {toast.timestamp}
                    </span>
                  </div>
                  <p className="text-[10px] text-zinc-300 font-sans leading-snug break-words">
                    {toast.message}
                  </p>
                </div>
              </div>
            ...
            {/* Compact orderInfo pill if present */}
            {toast.orderInfo && (
              <div className="bg-slate-900/90 border border-slate-800 rounded-md px-2 py-1 flex items-center justify-between text-[10px] font-mono">
                <div className="flex items-center gap-1.5 truncate">
                  ...
                  <span className="font-bold text-white truncate max-w-[80px]">{toast.orderInfo.name}</span>
                </div>

                <div className="flex items-center gap-1 text-zinc-200 shrink-0">
                  <span>{(toast.orderInfo.qty ?? 0).toLocaleString()}주</span>
                  <span>@</span>
