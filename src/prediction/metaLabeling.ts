...
export interface MetaLabelingInput {
  ...
  calibratedResult: CalibratedProbabilityOutput;
...
export class MetaLabelingFilter {
  ...
  public static evaluate(input: MetaLabelingInput): MetaLabelingOutput {
    ...
    const confidenceThresholdPass = input.calibratedResult.calibratedProbability >= minHitRateThreshold;
    ...
    const basePosition = decision === "BUY_READY" ? Math.min(25, Math.max(5, Math.round((input.calibratedResult.calibratedProbability - 50) * 0.6))) : 0;

    return {
      decision,
      executionProbability: decision === "BUY_READY" ? input.calibratedResult.calibratedProbability : 0,
