...
test("Bollinger squeeze compares current bandwidth with previous bars only", () => {
  const wide = Array.from({ length: 35 }, (_, index) => index % 2 === 0 ? 94 : 106);
  const narrow = Array.from({ length: 25 }, (_, index) => index % 2 === 0 ? 99.9 : 100.1);
  ...
  assert.ok(last.bandwidthPct < series[35].bandwidthPct);
});

test("closed upper-band cross is confirmed, forming cross stays candidate only", () => {
  const wide = Array.from({ length: 30 }, (_, index) => index % 2 === 0 ? 95 : 105);
  const narrow = Array.from({ length: 25 }, (_, index) => index % 2 === 0 ? 99.9 : 100.1);
